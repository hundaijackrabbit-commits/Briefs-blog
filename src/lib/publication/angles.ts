...
import { canUseConnectionAngle,canUseStrongestFactAngle,currentChangeThesis } from "@/lib/publication/angle-integrity";
...
export async function generateStoryAngles(graph:ResearchGraph,audience:PublicationAudience,keyword:string):Promise<StoryAngle[]>{
  ...
  if(graph.plan.intent==="current"||graph.plan.freshness==="live")candidates.push({key:"change",title:`What actually changed in ${subject}`,thesis:currentChangeThesis(subject,first?.predicate||""),claimIds:findings.slice(0,4).map(f=>f.id),risk:32});
  if(first&&canUseStrongestFactAngle(first.predicate))candidates.push({key:"strongest-fact",title:`The ${readablePredicate(first.predicate)} detail that changes how to read ${subject}`,thesis:`A single well-supported finding can be more useful than a broad recap; the article should explain that finding in context without borrowing source language.`,claimIds:[first.id,...findings.slice(1,3).map(f=>f.id)],risk:26});
  if(first&&second&&canUseConnectionAngle(first.predicate,second.predicate))candidates.push({key:"connection",title:`The connection inside ${subject} that is easy to miss`,thesis:`The useful story is the relationship between ${readablePredicate(first.predicate).toLowerCase()} and ${readablePredicate(second.predicate).toLowerCase()}, not a list of updates.`,claimIds:[first.id,second.id],risk:30});
