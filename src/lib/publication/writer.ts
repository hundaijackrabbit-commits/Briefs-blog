...
function deterministicDraft(graph:ResearchGraph,audience:PublicationAudience,category:string,contract:StoryContract):ArticleDraft{
  ...
  return {title:headlineFor(graph,contract),deck:deckFor(graph,domain,allEvidence),category,audience,articleType:articleType(graph),sections,claimIds:[...new Set([answer,evidenceSection,limitsSection].flatMap(section=>section.claimIds))],generatedBy:"briefs-deterministic"};
}
