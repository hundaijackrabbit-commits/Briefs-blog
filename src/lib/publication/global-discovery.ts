...
function bestResearchPhrase(titles:string[]){
  const subject=cleanTitle(titles[0]||"");
  const candidates=[...new Set(phraseCandidates(subject))];
  const ranked=candidates.map(phrase=>{
    const words=phrase.split(" ");
    const support=titles.reduce((count,title)=>{
      const set=tokenSet(title);
      return count+(words.every(word=>set.has(word))?1:0);
    },0);
    return {phrase,support,words:words.length};
  }).sort((a,b)=>b.support-a.support||b.words-a.words||b.phrase.length-a.phrase.length);
  const repeated=ranked.find(item=>item.support>=2&&item.words>=2);
  if(repeated)return repeated.phrase;
  const top=tokens(subject).slice(0,5).join(" ");
  return top||subject.slice(0,100);
}
