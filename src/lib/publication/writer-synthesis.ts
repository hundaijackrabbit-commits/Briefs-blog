...
export function detectWritingDomain(category:string,subject:string,evidenceText:string):WritingDomain{
  const c=category.toLowerCase();const text=`${subject} ${evidenceText}`.toLowerCase();
  if(/market|finance/.test(c)||/\b(bond|yield|treasury|stocks?|shares?|currency|central bank|interest rate|borrowing cost|oil price|credit)\b/.test(text))return "markets";
  if(/\b(health|medicine)\b/.test(c)||/\b(outbreak|infection|disease|virus|vaccine|hospital|mortality|transmission|cases)\b/.test(text))return "health";
  if(/policy/.test(c)||/\b(regulation|legislation|law|court|parliament|treaty|policy)\b/.test(text))return "policy";
  if(/technology/.test(c)||/\b(ai|artificial intelligence|semiconductor|software|cyber|technology|chip)\b/.test(text))return "technology";
