...
import { eventAlignmentComponents,eventTokens } from "@/lib/publication/event-identity";

const EVENT_CLASS_TERMS=new Set(["attack","ceasefire","sanction","elect","protest","regulate","merge","fail","move","outbreak","die","warn","resign","arrest","evacuate","deploy","traffic","expand","conflict"]);
function clamp(n:number){return Math.max(0,Math.min(100,Math.round(n)));}
function sourceText(source:ResearchSource,graph:ResearchGraph){
  const findings=graph.findings.filter(f=>f.sourceIds.includes(source.id)).map(f=>`${f.subject} ${f.predicate} ${f.valueText} ${f.statement}`).join(" ");
  return `${source.title} ${source.excerpt||""} ${findings}`;
}
...
function mismatchPenalty(anchor:ResearchEventAnchor,text:string,distinctiveCoverage:number){
  ...
  return Math.min(24,new Set(mismatches).size*8);
}
...
function alignmentForSource(source:ResearchSource,graph:ResearchGraph,anchor:ResearchEventAnchor){
  ...
  const semantic=Math.round(title.score*.62+full.score*.38);
  const negativePenalty=mismatchPenalty(anchor,sourceText(source,graph),full.distinctive);
  const baseScore=clamp(semantic*.86+temporal*.14-negativePenalty);
  return {sourceId:source.id,topic:full.topic,action:full.action,geography:full.geography,temporal,negativePenalty,baseScore,matched:matchedTermSet(anchor,sourceText(source,graph))};
}
...
export function evaluateResearchAlignment(args:{graph:ResearchGraph;anchor:ResearchEventAnchor;selectedSubject:string;queries:string[];clusterCoherence?:number;repaired?:boolean;}):ResearchSubjectAlignment{
  ...
  const prelimRelevant=prelim.filter(x=>x.raw.baseScore>=50&&x.raw.matched.size>0);
  ...
  const sourceScores:ResearchSourceAlignment[]=prelim.map(({source,raw})=>{
    ...
    for(const other of prelimRelevant){
      ...
      const common=sharedCount(raw.matched,other.raw.matched);
    ...
    const actionRequired=anchor.actionTerms.length===0||raw.action>0||raw.matched.size>=2;
    const temporalRequired=graph.plan.freshness==="historical"||raw.temporal>=45;
    const passed=score>=58&&raw.matched.size>0&&topicRequired&&actionRequired&&temporalRequired;
    const temporalStatus:ResearchSourceAlignment["temporalStatus"]=raw.temporal>=72?"current":raw.temporal>=45?"aging":"stale";
    return {sourceId:source.id,score,topic:raw.topic,action:raw.action,geography:raw.geography,temporal:raw.temporal,temporalStatus,corroboration,negativePenalty:raw.negativePenalty,matchedDistinctive:[...raw.matched],passed};
  });
  ...
  const alignedTexts=alignedSources.map(source=>new Set(eventTokens(sourceText(source,graph))));
  ...
  const score=clamp(average*.45+entityCoverage*.15+actionCoverage*.10+temporal*.10+coverageRatio*100*.10+familyScore*.05+coherence*.05);

  const reasons:string[]=[];
  if(alignedSources.length<3)reasons.push(`Only ${alignedSources.length} source(s) matched the selected event; at least 3 are required.`);
