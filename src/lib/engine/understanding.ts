...
export async function understandEvent(eventId:string){
  ...
  for(const d of docs) await sql`insert into semantic_annotations(document_id,frame,extractor_version) values(${d.id}::uuid,${sql.json(frame)},'rules-v3.1') on conflict(document_id,extractor_version) do nothing`;
