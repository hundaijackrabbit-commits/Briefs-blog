...
export default function BriefClient({initialSubject}:{initialSubject:string}){
  ...
  return <main className="brief-me-shell">
    ...
    {data&&<article className="brief-result" aria-live="polite">
      ...
      {data.result.quality&&data.result.quality.warnings.length>0&&<section className="brief-block quality-note"><h3>Quality notes</h3><ul className="brief-watch">{data.result.quality.warnings.map((w,i)=><li key={i}>{w}</li>)}</ul></section>}
      ...
      <section className="brief-followup"><p className="eyebrow">KEEP THE CONTEXT</p><form onSubmit={submitFollowup}><input value={followup} onChange={(e:ChangeEvent<HTMLInputElement>)=>setFollowup(e.target.value)} placeholder={`Ask a follow-up about ${data.result.subject}…`} aria-label="Follow-up question"/><button disabled={loading||!followup.trim()}>Ask →</button></form><small>Follow-ups carry forward the subject, evidence and conversation context instead of starting over.</small></section>
