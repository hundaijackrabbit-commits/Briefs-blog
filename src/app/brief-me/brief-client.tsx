...
type BriefContext={conversationId?:string;rootSubject:string;priorQueries:string[];entityHints:string[];claimIds:string[];sourceIds:string[]};
type BriefResponse={
...
export default function BriefClient({initialSubject}:{initialSubject:string}){
  ...
  return <main className="brief-me-shell">
    ...
    {data&&<article className="brief-result" aria-live="polite">
      ...
      <section className="brief-followup"><p className="eyebrow">KEEP THE CONTEXT</p><form onSubmit={submitFollowup}><input value={followup} onChange={e=>setFollowup(e.target.value)} placeholder={`Ask a follow-up about ${data.result.subject}…`} aria-label="Follow-up question"/><button disabled={loading||!followup.trim()}>Ask →</button></form><small>Follow-ups carry forward the subject, evidence and conversation context instead of starting over.</small></section>
