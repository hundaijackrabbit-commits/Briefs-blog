...
## Publication Engine 1.2.2 — Reader-Facing Distinctiveness History
- Reader-Facing Distinctiveness History — operational; only flagship rows that produced an article (`drafted` or `published`) influence future daily distinctiveness
- Blocked-Run History Exclusion — operational; `research-required` and selection-only ledger rows cannot suppress unrelated future stories
- Category Rotation De-Gating — operational; an unrelated story is not penalized merely because yesterday's reader-facing flagship used the same broad category
- Bounded Category Fatigue — operational; category concentration after more than two appearances in the latest five reader-facing flagships creates only a small capped nudge
...
## Publication Engine 1.2.2 — Bounded Interpretive Depth
- Bounded Interpretive Depth — operational; after unused grounded findings are exhausted, deterministic fallback drafts may add a small fixed set of epistemic connective sentences to reader-facing analysis
- No New Fact Injection — operational; connective prose describes the distinction between observed evidence and downstream uncertainty without asserting new event facts
- Grounded Expansion First — operational; unused research findings are always consumed before interpretive connective prose is considered
- Reader-Ready Gate Preserved — operational; the 160-word publication minimum remains unchanged
- Writer Safety Margin Preserved — operational; deterministic drafts continue to target 180 reader-facing words
