...
function inferIntent(subject:string,parts:string[]):ResearchIntent{
  ...
  if(/\b(history|historical|origin|timeline|ancient|century)\b/i.test(subject)) return "history";
...
export function decomposeResearchRequest(request:BriefRequest):ResearchQueryPlan{
  ...
  const freshness=request.freshnessRequirement==="historical"?"historical":request.freshnessRequirement==="recent"?"live":intent==="history"?"historical":intent==="current"||intent==="finance"?"live":"current";
